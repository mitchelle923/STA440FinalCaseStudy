...
model <- glm(mort30 ~ age+hour+dow+month+ moonphase, data = modelData, family = "binomial")
...
```{r echo=FALSE}
seven <- ggplot(data = SurgeryTiming, aes(x = mortality_rsi)) + geom_density(fill = "pink", color = "black", alpha = 0.7) +labs(title = "7) Distribution Risk Stratification Index (30-Day Mortality)",
...
```{r, echo=FALSE}
plot_grid(five, six, seven, eight, ncol=2, rel_heights = c(1.1, 2))
```
...
```{r, echo=FALSE}
plot_grid(nine, ten, eleven, twelve, ncol=2, rel_heights = c(1.1, 2))
```

*Image 9) Operation Hour*

The distribution is bimodal with a huge concentration of operation hours at around 8:00 am and a much smaller concentration at 12:30 pm.
...
```{r, echo=FALSE}
fourteen <- ggplot(SurgeryTiming, aes(x =mort30, y = age, fill = mort30)) + geom_boxplot() + labs(title = "14) Relationship between Age and 30-Day Mortality", x = "30 Day Mortality", y = "Age", fill = "30 Day Mortality") +theme(aspect.ratio = 1,legend.position = "none",
...
```{r, echo=FALSE}
fifteen <- ggplot(SurgeryTiming, aes(x =mort30, y = hour,fill = mort30)) + geom_boxplot() + labs(title = "15) Relationship between Hour of Operation and 30-Day Mortality", x = "30 Day Mortality", y = "Hour of Operation", fill = "30 Day Mortality") +theme(aspect.ratio = 1,legend.position = "none",
        plot.title = element_text(size = 8),
        plot.subtitle = element_text(size = 6),
        axis.text = element_text(size = 5),
        axis.title = element_text(size = 6))
...
```{r, echo=FALSE}
plot_grid(fourteen , fifteen, ncol=2, rel_heights = c(1.1, 2))
```

*Image 14) Age and 30 Day Morality*

The box plots show that operations of patients that have 30-day mortality are more generally of an older age than patients who do not have 30-day mortality.

*Image 15) Hour and 30 Day Morality*

The box plots show that operations of patients that have 30-day mortality tend to happen later in the day than operations of patients that do not have 30-day mortality.

```{r, echo=FALSE}
sixteen <- ggplot(data=SurgeryTiming, aes(x=mort30,fill = mort30)) +
    geom_bar() + labs(title = '16) Relationship between Day of Operation & 30-Day Mortality', x = '30-Day Mortality', y = 'Count', fill = "30 Day Mortality")+facet_wrap(~dow)+theme(aspect.ratio = 1,legend.position = "none",
        plot.title = element_text(size = 7),
        plot.subtitle = element_text(size = 6),
        axis.text = element_text(size = 5),
        axis.title = element_text(size = 6))
...
```{r, echo=FALSE}
seventeen <- ggplot(data=SurgeryTiming, aes(x=mort30,fill = mort30)) +
    geom_bar() + labs(title = '17) Relationship between Month of Operation & 30-Day Mortality', x = '30-Day Mortality', y = 'Count', fill = "30 Day Mortality")+facet_wrap(~month) +theme(aspect.ratio = 1,legend.position = "none",
        plot.title = element_text(size = 7),
        plot.subtitle = element_text(size = 6),
        axis.text = element_text(size = 5),
        axis.title = element_text(size = 6))
...
```{r, echo=FALSE}
plot_grid(sixteen , seventeen, ncol=2, rel_heights = c(1.1, 2))
```

```{r, echo=FALSE}
dow_mort <- SurgeryTiming %>%
  group_by(dow, mort30) %>%
  summarise(
    Count = n())
dow_mort %>% 
...
*Image 16) Day of Week and 30 Day Mortality*

The day with the highest proportion of 30-day mortality cases is Friday, then Tuesday, then Monday, then Thursday, then Wednesday.

*Image 17) Month and 30 Day Mortality*

The month with the highest proportion of 30-day mortality cases is January, then February, then July.
